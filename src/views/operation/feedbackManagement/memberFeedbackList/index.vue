            ...
            <el-row :gutter="24">
                ...
                <el-col :md="12" :lg="6">
                    ...
                    <el-form-item :label="$t('sellerFeedbackListTable.feedbackCategoryName')" prop="feedbackCategoryName">
                        <el-select v-model="searchForm.feedbackCategoryName" :placeholder="$t('pleaseSelect')">
                ...
                <el-col :md="12" :lg="6">
                    <!-- 会员-->
                    <el-form-item :label="$t('memberFeedbackList.userId')" prop="userId">
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10 between">
            ...
            <div>
                ...
                <!-- 标记已读 -->
                <el-button v-permission="'feedbackManagement_memberFeedbackList_read'" @click="handleRead">
                    {{ $t("memberFeedbackList.markAsRead") }}
                </el-button>
            </div>
            <div>
                <!-- 导出 -->
                <el-button v-permission="'feedbackManagement_memberFeedbackList_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" :column="tableColumn" @row-click="handleRowClick">
            ...
            <!-- 来源 -->
            <BmTableColumn :label="$t('sellerFeedbackListTable.source')" prop="source">
            ...
            <BmTableColumn :label="$t('table.operation')">
                <template #default="scope">
                    <el-button v-permission="'feedbackManagement_memberFeedbackList_look'" type="text" @click.stop="handleAddEdit(scope.row)">{{ $t("table.detail") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>

        <BmPagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="pageSize" :total="pageTotal"></BmPagination>
        <!-- 查看详情弹窗 -->
    ...
    setup() {
        ...
        const handleExportExcel = (val) => {
            feedbackMessageExport(searchForm)
                .then((res) => {
                    exportExcel("MemberFeedbackList", res);
