<template>
    <div class="plr-15">
        ...
        <el-carousel
            v-if="datas.componentDetails && datas.componentDetails.length > 3"
            ref="carouselRef"
        ...
        >
            <el-carousel-item v-for="(citem, index) in componentDetails" :key="index">
                <el-row :gutter="15">
                    <el-col v-for="(item, index) in citem" :key="index" :span="8">
                        <BmImage :style="'width: 100%; height: ' + (isPreview ? '102px' : '102px')" :src="item.goodsPic || require('@/assets/img/default.png')" fit="cover" />
        ...
        <el-row v-else-if="datas.componentDetails && datas.componentDetails.length > 0" :gutter="15">
            <el-col v-for="(item, index) in datas.componentDetails" :key="index" :span="8">
                <BmImage :style="'width: 100%; height: ' + (isPreview ? '102px' : '102px')" :src="item.goodsPic || require('@/assets/img/default.png')" fit="cover" />
                <div class="plr-10 bg-white">
                    <div class="pt-10 lh-12 hidden-2" style="height:38px">{{ item.goodsTitle }}</div>
                    <div class="ptb-10 red">{{ $store.state.app.currencySymbol }} {{ item.goodsPrice || "0.00" }}</div>
        ...
        <el-row v-else :gutter="15">
            <el-col v-for="(item, index) in 3" :key="index" :span="8">
                <BmImage :style="'width: 100%; height: ' + isPreview ? '102px' : '102px'" :src="require('@/assets/img/default.png')" fit="cover" />
                <div class="plr-10 bg-white">
        ...
        <!-- <div v-if="componentDetails.length > 0">
            <el-carousel ref="carouselRef" height="200px" :indicator-position="datas.effect === 2 ? 'none' : ''" arrow="never">
                <el-carousel-item v-for="(citem, index) in componentDetails" :key="index">
                    <el-row :gutter="15">
                        <el-col v-for="(item, index) in citem" :key="index" :span="8">
                            <BmImage style="width: 100%; height: 102px" :src="item.goodsPic || require('@/assets/img/default.png')" fit="cover"/>
                            <div class="pt-10 lh-12 hidden-2" style="height:38px">{{ item.goodsTitle }}</div>
                            <div class="ptb-10 red">{{ $store.state.app.currencySymbol }} {{ item.goodsPrice || "0.00" }}</div>
                        </el-col>
                    </el-row>
        ...
        <el-row v-else :gutter="15">
            <el-col v-for="(item, index) in 3" :key="index" :span="8">
                <BmImage style="width: 100%; height: 102px" :src="require('@/assets/img/default.png')" fit="cover"/>
                <div class="pt-10 lh-12 hidden-2" style="height:38px">{{ $t("storeDesign.goodTitle") }}</div>
                <div class="ptb-10 red">{{ $store.state.app.currencySymbol }} 0.00</div>
            </el-col>
        </el-row> -->
        ...
        watch(
            ...
            (nVal) => {
                ...
                if (props.datas.effect == 2) {
                    result = props.datas.componentDetails.slice(0, 3); //无轮播 截取前3个
                } else {
                    //每6个一组
                    if (props.datas.componentDetails.length > 3) {
                        for (let i = 0; i < props.datas.componentDetails.length; i += 3) {
                            result.push(props.datas.componentDetails.slice(i, i + 3));
                        }
                    } else {
        ...
        watch(
            ...
            (nVal) => {
                ...
                if (nVal == 2) {
                    ...
                    result = props.datas.componentDetails.slice(0, 3); //前3个
                } else {
                    //每3个一组
                    if (props.datas.componentDetails.length > 3) {
                        for (let i = 0; i < props.datas.componentDetails.length; i += 3) {
