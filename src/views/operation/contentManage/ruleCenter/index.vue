<template>
    ...
    <div class="p-25 bg-white round-4">
        <div class="w-200 fl">
            <!-- 新增一级分类 -->
            <el-button v-permission="'contentManage_ruleCenter_addClassification'" @click="handleAddEditNav('add')" type="primary" plain icon="el-icon-plus" class="w-100 mb-20">
                ...
                <template #default="{ data,node }">
                    <div @click="handleChangeNav(data)" class="flex flex1 between vcenter">
                        ...
                        <div class="flex1" style="flex: 0 0 40px">
                            <el-button
                                v-permission="'contentManage_ruleCenter_delClassification'"
                            ...
                            ></el-button>
                            <el-button
                                v-permission="'contentManage_ruleCenter_addClassification'"
        ...
        <div style="margin-left:220px">
            <div class="flex between mb-10">
                <div>
                    <div class="fl mb-10 mr-15">
                        <!-- 新增 -->
                        <el-button v-permission="'contentManage_ruleCenter_add'" @click="handleAddEdit('add')" type="primary" icon="el-icon-plus">
                            {{ $t("status.add") }}
                        </el-button>
                    </div>
                    <div class="fl mb-10">
                        ...
                        <!-- 启用 -->
                        <el-button v-permission="'contentManage_ruleCenter_enable'" @click="handleOpenShield(1)">
                            {{ $t("status.enable") }}
                        </el-button>
                        <!-- 禁用 -->
                        <el-button v-permission="'contentManage_ruleCenter_disable'" @click="handleOpenShield(0)">
                            {{ $t("status.forbidden") }}
                        </el-button>
                        <!-- 删除 -->
                        <el-button v-permission="'contentManage_ruleCenter_del'" @click="handleDelete">{{ $t("status.delete") }}</el-button>
                    </div>
                </div>
            ...
            <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick">
                ...
                <BmTableColumn :label="$t('table.operation')">
                    <template #default="scope">
                        <!-- 编辑 -->
                        <el-button v-permission="'contentManage_ruleCenter_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">
                            {{ $t("status.edit") }}
                        </el-button>
                        <!-- 删除 -->
                        <el-button v-permission="'contentManage_ruleCenter_del'" @click.stop="handleDelete(scope.row)" type="text">
                            {{ $t("status.delete") }}
                        </el-button>
                        <!-- 启用 -->
                        <el-button v-permission="'contentManage_ruleCenter_enable'" v-if="scope.row.status === 0" @click.stop="handleOpenShield(1, scope.row)" type="text">
                            {{ $t("status.enable") }}
                        </el-button>
                        <!-- 禁用 -->
                        <el-button v-permission="'contentManage_ruleCenter_disable'" v-if="scope.row.status === 1" @click.stop="handleOpenShield(0, scope.row)" type="text">
                            {{ $t("status.forbidden") }}
                        </el-button>
                        <!-- 详情 -->
                        <el-button v-permission="'contentManage_ruleCenter_look'" @click.stop="handleDetail(scope.row)" type="text">
                            {{ $t("table.detail") }}
                        </el-button>
                    </template>
                </BmTableColumn>
...
export default {
    ...
    setup() {
        ...
        const mainType = 2; //类型 1视频分类 2规则中心 3帮助中心
