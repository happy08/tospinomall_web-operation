<template>
        ...
        <el-form ref="formRef" :model="form" label-width="auto">
            ...
            <div class="p-20 bg-light-grey">
                ...
                <el-form-item v-if="form.releaseType == 2" :label="$t('contentMsg.setTime')" prop="setTime" :rules="rulesCommon.required">
                    <el-date-picker v-model="form.setTime" type="datetime" :placeholder="$t('contentMsg.setTime')" :disabled-date="disabledDate"> </el-date-picker>
                </el-form-item>
            </div>
        </el-form>
    </BmDialog>
    ...
    <!-- 指定店铺列表 -->
    <SelectionSellerList v-model:visible="memberVisible" :ids="form.objectIds" @change="onChangeObjectIds" />
</template>

<script>
import { ref, getCurrentInstance, watch, computed } from "vue";
import { messagePushMallAddMsg, messagePushMallUpdateById } from "@/api/operation";
import SelectionSellerList from "@/components/selectionSellerList";
import dayjs from "dayjs";

export default {
    name: "storeDesignAdd",
    components: { SelectionSellerList },
    props: {
        visible: Boolean,
        row: Object,
        treeData: Array
    ...
    setup(props, { emit }) {
        ...
        const objectTypeList = computed(() => {
            return {
                1: proxy.$t("contentMsg.allStore"),
                2: proxy.$t("contentMsg.assignStore")
            };
        });
        //接收平台
            ...
            formRef.value.validate((valid) => {
                if (valid) {
                    ...
                    if (form.value.releaseType == 1 && form.value.objectType == 2 && form.value.objectIds == "") {
                        proxy.$message.error(proxy.$t("contentMsg.selectStore"));
                        return;
                    }
