<template>
    ...
    <div class="p-25 bg-white round-4">
        <div class="w-200 fl">
            <!-- 新增一级分类 -->
            <el-button v-permission="'contentManage_sellerMsg_addClassification'" @click="handleAddEditNav('add')" type="primary" plain icon="el-icon-plus" class="w-100 mb-20">
                ...
                <template #default="{ data,node }">
                    <div @click="handleChangeNav(data)" class="flex flex1 between vcenter">
                        ...
                        <div class="flex1" style="flex: 0 0 20px">
                            <el-button v-permission="'contentManage_sellerMsg_editClassification'" @click.stop="handleAddEditNav('edit', data)" icon="el-icon-edit" type="text"></el-button>
                            <el-button
                                v-permission="'contentManage_sellerMsg_delClassification'"
        ...
        <div style="margin-left:220px">
            <div class="flex between mb-10">
                <div>
                    ...
                    <div class="fl mb-10 mr-15">
                        <!-- 新增 -->
                        <el-button v-permission="'contentManage_sellerMsg_add'" @click="handleAddEdit('add')" type="primary" plain icon="el-icon-plus">
                            {{ $t("status.add") }}
                        </el-button>
                    </div>
                    <div class="fl mb-10">
                        ...
                        <!-- 发布 -->
                        <el-button v-permission="'contentManage_sellerMsg_release'" @click="handlePublish">
                            {{ $t("contentMsg.publish") }}
                        </el-button>

                        <!-- 删除 -->
                        <el-button v-permission="'contentManage_sellerMsg_del'" @click="handleDelete">{{ $t("status.delete") }}</el-button>
                    </div>
                </div>

                <div class="flex vstart">
                    <!-- 导出 -->
                    <el-button v-permission="'contentManage_sellerMsg_export'" @click="handleExportExcel">{{ $t("table.exportExcel") }}</el-button>
                </div>
            </div>
            ...
            <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick">
                ...
                <BmTableColumn :label="$t('table.operation')" fixed="right" width="160px">
                    <template #default="scope">
                        <!-- 编辑 -->
                        <el-button v-permission="'contentManage_sellerMsg_edit'" v-if="scope.row.status == 0" @click.stop="handleAddEdit('edit', scope.row)" type="text">
                            {{ $t("status.edit") }}
                        </el-button>
                        <!-- 删除 -->
                        <el-button v-permission="'contentManage_sellerMsg_del'" v-if="scope.row.status == 0" @click.stop="handleDelete(scope.row)" type="text">
                            {{ $t("status.delete") }}
                        </el-button>
                        <!-- 发布 (0未发布 1已发布 3已作废 )-->
                        <el-button v-permission="'contentManage_sellerMsg_release'" v-if="scope.row.status == 0" @click.stop="handlePublish(scope.row)" type="text">
                            {{ $t("contentMsg.publish") }}
                        </el-button>
                        <!-- 查看 -->
                        <el-button v-permission="'contentManage_sellerMsg_look'" v-if="scope.row.status != 0" @click.stop="handleDetail(scope.row)" type="text">
                            {{ $t("table.detail") }}
                        </el-button>
                    </template>
                </BmTableColumn>
...
export default {
    ...
    setup() {
        ...
        const mainType = 2; //类别 1会员消息推送分类 2卖家消息推送分类
