<template>
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10" style="justify-content: flex-end">
            <div class="flex">
                <!-- 导出 -->
                <el-button v-permission="'memberFunds_copingManagement_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <BmTableColumn :label="$t('copingManagementTable.buyerInfo')" align="center" prop="buyerInfo">
                <template #default="scope">
                    <span class="block">{{ scope.row.buyerName }}</span>
            ...
            <BmTableColumn :label="$t('table.operation')" fixed="right" width="150" align="center">
                <template #default="scope">
                    <!-- 退款(状态为待付款0) -->
                    <el-button v-permission="'memberFunds_copingManagement_refund'" v-if="scope.row.status == 0" @click.stop="handleRefund(scope.row)" type="text">{{
                        $t("copingManagement.returnMoney")
                    }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
...
import { documentTypeList, documentTypeListExport } from "@/api/finance";
...
export default {
    ...
    setup() {
        ...
        const handleExportExcel = (val) => {
            documentTypeListExport(searchForm)
                .then((res) => {
                    exportExcel("PayableManagement", res);
