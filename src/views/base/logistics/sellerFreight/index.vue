            ...
            <el-row :gutter="24">
                <el-col :md="12" :lg="6">
                    ...
                    <el-form-item :label="$t('sellerFreightTable.tempName')" prop="tempName">
                        <el-input v-model="searchForm.tempName" :placeholder="$t('sellerFreightTable.tempName')" clearable></el-input>
                    </el-form-item>
                </el-col>

                <el-col :md="12" :lg="6">
                    <!-- 操作日期 -->
                    <el-form-item :label="$t('sellerFreightTable.updateTime')" prop="_tmpTimeValue">
                        <el-date-picker
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex between mb-10">
            <div>
                <div class="fl mb-10">
                    <!-- 新增 -->
                    <el-button v-permission="'logistics_sellerFreight_add'" @click="handleAddEdit('add')" type="primary" icon="el-icon-plus">
                        {{ $t("status.add") }}
                    </el-button>
                    <!-- 编辑
            ...
            <div class="flex vstart">
                <!-- 表格选项 -->
                <BmTableOption table="sellerFreightTable" :column="tableColumn" class="ml-10" />
            </div>
        </div>
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <BmTableColumn prop="id" :label="$t('sellerFreightTable.id')"> </BmTableColumn>
            <BmTableColumn prop="tempName" :label="$t('sellerFreightTable.tempName')"> </BmTableColumn>
            <BmTableColumn prop="tradeType" :label="$t('sellerFreightTable.tradeType')">
                <template #default="scope">
                    {{ scope.row.tradeTypeName }}
                </template>
            </BmTableColumn>
            <BmTableColumn prop="freightType" :label="$t('sellerFreightTable.freightType')">
                <template #default="scope">
                    {{ scope.row.freightTypeName }}
                </template>
            </BmTableColumn>
            <BmTableColumn prop="sendType" :label="$t('sellerFreightTable.sendType')">
                <template #default="scope">
                    {{ scope.row.sendTypeName }}
                </template>
            </BmTableColumn>
            <BmTableColumn prop="status" :label="$t('sellerFreightTable.status')" width="120"
                ><template #default="scope">
                    {{ scope.row.statusName }}
                </template>
            </BmTableColumn>
            <BmTableColumn prop="createTime" :label="$t('sellerFreightTable.createTime')" width="155"></BmTableColumn>
            <BmTableColumn prop="updateTime" :label="$t('sellerFreightTable.updateTime')" width="155"></BmTableColumn>
            <BmTableColumn prop="remark" :label="$t('sellerFreightTable.remark')"> </BmTableColumn>
            <BmTableColumn :label="$t('table.operation')" width="150" fixed="right">
                <template #default="scope">
                    <el-button v-permission="'logistics_sellerFreight_disable'" v-if="scope.row.status === 0" @click.stop="handleOpenShield(1, scope.row)" type="text">{{
                        $t("status.forbidden")
                    }}</el-button>
                    <el-button v-permission="'logistics_sellerFreight_enable'" v-else @click.stop="handleOpenShield(0, scope.row)" type="text">
                        {{ $t("status.enable") }}
                    </el-button>
                    <el-button v-permission="'logistics_sellerFreight_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">
                        {{ $t("status.edit") }}
                    </el-button>
                    <el-button v-permission="'logistics_sellerFreight_del'" @click.stop="handleDelete(scope.row)" type="text">
                        {{ $t("status.delete") }}
                    </el-button>
                </template>
            </BmTableColumn>
...
import { logisticsSellerFindByConditions, logisticsSellerDelete, logisticsSellerUpdateStatus } from "@/api/base";
import { dateShortcuts } from "@/utils/dateShortcuts";
import AddEdit from "./component/addEdit";

export default {
    name: "BaseSellerFreight",
    components: { AddEdit },
    setup() {
        ...
        //表格选项的列
        const tableColumn = ["id", "tempName", "tradeType", "freightType", "sendType", "status", "createTime", "updateTime", "remark"];

        //获取列表
        const getListPage = () => {
            logisticsSellerFindByConditions(searchForm)
                .then((res) => {
        ...
        const handleOpenShield = (status, row) => {
            ...
            logisticsSellerUpdateStatus(params)
                .then((res) => {
                    initData();
                    proxy.$message.success(res.msg);
                })
