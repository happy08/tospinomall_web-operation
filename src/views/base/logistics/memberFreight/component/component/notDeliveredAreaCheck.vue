<template>
    <BmDialog v-model:visible="visible" :title="$t('memberShippingTemplate.nonDeliveryArea')" :confirm="confirm" :cancel="cancel" width="80vw">
        <!-- 地区选择 -->
        ...
        <!-- 不配送原因选择弹窗
        <BmDialog v-model:visible="timeVisible" :title="$t('memberShippingTemplate.reasonsForNonDelivery')" :confirm="deliveryTimeConfirm" :cancel="deliveryTimeCancel" width="800px">
            <div class="flex">
                <div style="width:20%">
                    {{ $t("memberShippingTemplate.reasonsForNonDelivery") }}
                </div>
                <div>
                    <el-checkbox-group v-model="copyAreaItem.nonDeliveryCause">
                        <el-checkbox :label="item" v-for="(item, index) in noSendReasonArr" :key="index"></el-checkbox>
                    </el-checkbox-group>
                </div>
            </div>
        </BmDialog> -->
...
export default {
    ...
    emits: ["update:visible", "updateNotDeliveredProvince"],
    setup(props, { emit }) {
        ...
        // 不配送原因对应数据
        const noSendReasonArr = [
            proxy.$t("memberShippingTemplate.notDeliveredReason1"),
            proxy.$t("memberShippingTemplate.notDeliveredReason2"),
            proxy.$t("memberShippingTemplate.notDeliveredReason3"),
            proxy.$t("memberShippingTemplate.notDeliveredReason4"),
            proxy.$t("memberShippingTemplate.notDeliveredReason5")
        ];
        // 选中省份的数量
        ...
        const proveinceChooseJudge = (val) => {
            for (let i = 0; i < val.length; i++) {
                // 省
                if (val[i].ck) {
                    provinceChooseArr.value.push(val[i].name);
                } else {
        ...
        const confirm = () => {
            ...
            setTimeout(() => {
                // 每次都先将省份选择的数量清空
                provinceChooseArr.value = [];
                provinceChooseNum.value = 0;
                proveinceChooseJudge(form.value);
                enableArea(form.value);
                emit("update:visible", false);
                emit("updateNotDeliveredProvince", provinceChooseArr.value, form.value);
                loading.close();
            }, 100);
        };
        const cancel = () => {
