<template>
            ...
            <el-row :gutter="24">
                <el-col :md="12" :lg="6">
                    ...
                    <el-form-item :label="$t('afterSalesReasonSetting.causeType')" prop="title">
                        <el-select v-model="searchForm.title" :placeholder="$t('pleaseSelect')">
                            <el-option v-for="item in ReasonSelectList" :key="item.value" :label="item.label" :value="item.label"></el-option>
                        </el-select>
                    </el-form-item>
                    <!-- 注册日期 -->
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex between mb-10">
            <div>
                ...
                <div class="fl mb-10">
                    ...
                    <!-- 仅退款 -->
                    <el-button @click="handleSelect(0)">
                        {{ $t("afterSalesReasonSetting.refundOnly") + "(" + countList.refundOnly + ")" }}
                    </el-button>
                    <!-- 退货退款 -->
                    <el-button @click="handleSelect(1)">
                        {{ $t("afterSalesReasonSetting.returnRefund") + "(" + countList.returnRefund + ")" }}
                    </el-button>
                </div>
                <!-- 功能操作区域 -->
                <div class="fl mb-10 ml-50">
                    ...
                    <!-- 新增 -->
                    <el-button v-permission="'transactionSetting_workOrderReasonSetting_add'" @click="handleAddEdit('add')" type="primary" icon="el-icon-plus">{{ $t("status.add") }}</el-button>
                    <!-- 编辑 -->
          ...
          {{ $t('table.exportExcel') }}
                ...
                <BmTableOption table="afterSalesReasonSettin    gTable" :column="tableColumn" class="ml-10" />
            </div>
        </div>
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-dblclick="handleRowdblClick" :column="tableColumn">
            <BmTableColumn prop="workOrderType" :label="$t('workReturn.workOrderType')" width="150">
                <template #default="scope">{{ scope.row.applyTypeName }}</template>
            </BmTableColumn>
            ...
            <BmTableColumn :label="$t('table.operation')" width="150" align="center">
                <template #default="scope">
                    <el-button v-permission="'transactionSetting_workOrderReasonSetting_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">{{ $t("status.edit") }}</el-button>
                    <el-button v-permission="'transactionSetting_workOrderReasonSetting_del'" @click.stop="handleDelete(scope.row)" type="text">{{ $t("status.delete") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
    ...
    <!-- 新增编辑 -->
    <AddEdit v-model:visible="editVisible" :row="editRow" :selectData="ReasonSelectList" @reloadData="initData" />
</template>
...
export default {
    ...
    setup() {
        ...
        // select选择数据
        const ReasonSelectList = [
            { value: 1, label: proxy.$t("afterSalesReasonSetting.onlyReason1") },
            { value: 2, label: proxy.$t("afterSalesReasonSetting.onlyReason2") },
            { value: 3, label: proxy.$t("afterSalesReasonSetting.onlyReason3") },
            { value: 4, label: proxy.$t("afterSalesReasonSetting.onlyReason4") },
            { value: 5, label: proxy.$t("afterSalesReasonSetting.onlyReason5") },
            { value: 6, label: proxy.$t("afterSalesReasonSetting.onlyReason6") },
            { value: 7, label: proxy.$t("afterSalesReasonSetting.onlyReason7") },
            { value: 8, label: proxy.$t("afterSalesReasonSetting.onlyReason8") },
            { value: 9, label: proxy.$t("afterSalesReasonSetting.onlyReason9") },
            { value: 10, label: proxy.$t("afterSalesReasonSetting.onlyReason10") }
        ];
        //快捷日期选项
        ...
        //搜索
        const handleSearch = () => {
            params.value.endTime = proxy.utils.formatStandardDate(searchForm._tmpTimeValue && searchForm._tmpTimeValue[1], true);
            params.value.startTime = proxy.utils.formatStandardDate(searchForm._tmpTimeValue && searchForm._tmpTimeValue[0], true);
        ...
        const resetForm = () => {
            ...
            params.value.endTime = params.value.startTime = "";
        ...
        //表格选项的列
        const tableColumn = ["workOrderType", "applyReason", "sort", "status", "create", "lastChange"];
        // 用来存储仅退款和退货退款信息条数的对象
        const countList = ref({
            // 仅退款条数
            refundOnly: 0,
