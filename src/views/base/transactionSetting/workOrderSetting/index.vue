<template>
    ...
    <div class="p-25 bg-white round-4">
        <BmTable ref="multipleTable" :data="tableData" :span-method="arraySpanMethod" :default-sort="{ prop: 'tradeType', order: 'ascending' }">
            ...
            <!-- 支付方式描述 -->
            <BmTableColumn prop="description" :label="$t('workSettingTable.configurationType')" width="320"> </BmTableColumn>
            <!-- 时间 -->
            <BmTableColumn prop="value" :label="$t('workSettingTable.time')" width="200">
                <template #default="scope">
            ...
            <!-- 备注 -->
            <BmTableColumn prop="remark" :label="$t('workSettingTable.remark')">
                <template #default="scope">
                    {{ scope.row.remark }}
                </template>
            </BmTableColumn>
            <BmTableColumn :label="$t('table.operation')" width="100" align="center">
                <template #default="scope">
                    <el-button @click="handleSave(scope.row)" type="text" :disabled="disabled">{{ $t("workSettingTable.save") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
    </div>
