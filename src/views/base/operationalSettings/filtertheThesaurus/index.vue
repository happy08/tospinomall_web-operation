<template>
    ...
    <div class="top-search-form">
        <BmBreadcrumb />
        <BmAlert :title="$t('filterWords.filterWordsTips')" icon="warning" class="mb-20" />
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10 between">
            <div>
                <!-- 新增 -->
                <el-button v-permission="'operationalSettings_filtertheThesaurus_add'" type="primary" @click="handleAddEdit('add')">+{{ $t("status.add") }}</el-button>
                <!-- 功能操作 -->
                <span class="mlr-30">{{ $t("status.functionalOperation") }}:</span>
                <!-- 批量删除 -->
                <el-button v-permission="'operationalSettings_filtertheThesaurus_del'" @click="handleAddEdit('allDelete')">{{ $t("filterWords.batchDeletion") }}</el-button>
                <!-- 批量禁用 -->
                <el-button v-permission="'operationalSettings_filtertheThesaurus_disable'" @click="handleAddEdit('allDisable')">{{ $t("filterWords.batchDisable") }}</el-button>
                <!-- 批量启用 -->
                <el-button v-permission="'operationalSettings_filtertheThesaurus_enable'" @click="handleAddEdit('allEnable')">{{ $t("filterWords.batchEnable") }}</el-button>
            </div>
            <div class="flex">
                <!-- 导出 -->
                <el-button v-permission="'operationalSettings_filtertheThesaurus_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <!-- 过滤词名次 -->
            <BmTableColumn :label="$t('filterWordsTable.name')" prop="name">
            ...
            <BmTableColumn :label="$t('table.operation')" fixed="right" align="center">
                <template #default="scope">
                    <!--启用-->
                    <el-button v-permission="'operationalSettings_filtertheThesaurus_enable'" v-if="scope.row.status == 1" @click.stop="handleAddEdit('enable', scope.row)" type="text">{{
                        $t("status.enable")
                    }}</el-button>
                    <!--禁用-->
                    <el-button v-permission="'operationalSettings_filtertheThesaurus_disable'" v-if="scope.row.status == 0" @click.stop="handleAddEdit('disable', scope.row)" type="text">{{
                        $t("status.forbidden")
                    }}</el-button>
                    <!-- 编辑 -->
                    <el-button v-permission="'operationalSettings_filtertheThesaurus_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">{{ $t("status.edit") }}</el-button>
                    <!-- 删除 -->
                    <el-button v-permission="'operationalSettings_filtertheThesaurus_del'" @click.stop="handleAddEdit('delete', scope.row)" type="text">{{ $t("status.delete") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
...
import { filterThesaurus, enableAllFilterWords, deleteAllFilterWords } from "@/api/base";
