<template>
    <div class="top-search-form">
        <BmBreadcrumb />
        <!-- <BmAlert
            :title="$t('searchRecommendedWords.searchRecommendedWordsTips')"
            icon="warning"
            class="mb-20"
        /> -->
            ...
            <el-row :gutter="24">
                ...
                <el-col :md="12" :lg="6">
                    <el-form-item :label="$t('searchJumpTable.keyword')" prop="keyword">
                        <el-input v-model="searchForm.keyword" :placeholder="$t('pleaseEnter')"> </el-input>
                    </el-form-item>
                </el-col>
            </el-row>
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10 between">
            <div>
                <!-- 新增 -->
                <el-button v-permission="'operationalSettings_searchJump_add'" type="primary" @click="handleAddEdit('add')">+{{ $t("status.add") }}</el-button>
                <!-- 功能操作 -->
                <span class="mlr-30">{{ $t("status.functionalOperation") }}:</span>
                <!-- 批量删除 -->
                <el-button v-permission="'operationalSettings_searchJump_del'" @click="handleAddEdit('allDelete')">{{ $t("filterWords.batchDeletion") }}</el-button>
                <!-- 批量禁用 -->
                <el-button v-permission="'operationalSettings_searchJump_disable'" @click="handleAddEdit('allDisable')">{{ $t("filterWords.batchDisable") }}</el-button>
                <!-- 批量启用 -->
                <el-button v-permission="'operationalSettings_searchJump_enable'" @click="handleAddEdit('allEnable')">{{ $t("filterWords.batchEnable") }}</el-button>
            </div>
            <div class="flex">
                <!-- 导出 -->
                <el-button v-permission="'operationalSettings_searchJump_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
                <BmTableOption table="searchJumpTable" :column="tableColumn" class="ml-10" />
            </div>
        </div>
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <!-- 关键字 -->
            <BmTableColumn :label="$t('searchJumpTable.keyword')" prop="keyword">
                <template #default="scope">{{ scope.row.keyword }}</template>
            </BmTableColumn>
            <!-- 落地页 -->
            <BmTableColumn :label="$t('searchJumpTable.jumpPage')" prop="jumpPage">
                <template #default="scope">{{ scope.row.jumpPage }}</template>
            </BmTableColumn>
            <!-- 跳转参数 -->
            <BmTableColumn :label="$t('searchJumpTable.jumpParam')" prop="jumpParam">
                <template #default="scope">{{ scope.row.jumpParam }}</template>
            </BmTableColumn>
            <!-- 权重值 -->
            <BmTableColumn :label="$t('searchJumpTable.weight')" prop="weight">
                <template #default="scope">{{ scope.row.weight }}</template>
            </BmTableColumn>
            <!-- 有效时间 -->
            <BmTableColumn :label="$t('searchJumpTable.validTime')" prop="validTime">
                <template #default="scope">
                    {{ scope.row.validTime }}
                </template>
            </BmTableColumn>
            <!-- 截至时间 -->
            <BmTableColumn :label="$t('searchJumpTable.deadTime')" prop="deadTime">
                <template #default="scope">
                    {{ scope.row.deadTime }}
                </template>
            </BmTableColumn>
            <!-- 状态 -->
            <BmTableColumn :label="$t('searchJumpTable.status')" prop="status">
                <template #default="scope">
                    {{ scope.row.statusName }}
                </template>
            </BmTableColumn>
            <!-- 添加时间 -->
            <BmTableColumn :label="$t('searchJumpTable.createTime')" prop="createTime">
                <template #default="scope">
                    {{ scope.row.createTime }}
                </template>
            </BmTableColumn>
            <!-- 更新时间 -->
            <BmTableColumn :label="$t('searchJumpTable.updateTime')" prop="updateTime">
                <template #default="scope">
            ...
            <BmTableColumn :label="$t('table.operation')" fixed="right" align="center">
                <template #default="scope">
                    <!--启用-->
                    <el-button v-permission="'operationalSettings_searchJump_enable'" v-if="scope.row.status == 1" @click.stop="handleAddEdit('enable', scope.row)" type="text">{{
                        $t("status.enable")
                    }}</el-button>
                    <!--禁用-->
                    <el-button v-permission="'operationalSettings_searchJump_disable'" v-if="scope.row.status == 0" @click.stop="handleAddEdit('disable', scope.row)" type="text">{{
                        $t("status.forbidden")
                    }}</el-button>
                    <!-- 编辑 -->
                    <el-button v-permission="'operationalSettings_searchJump_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">{{ $t("status.edit") }}</el-button>
                    <!-- 删除 -->
                    <el-button v-permission="'operationalSettings_searchJump_del'" @click.stop="handleAddEdit('delete', scope.row)" type="text">{{ $t("status.delete") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
...
import { searchJump, enableAllSearchJump, disableAllSearchJump, deleteAllSearchJump } from "@/api/base";
...
export default {
    ...
    setup() {
        ...
        let tableData = ref([]);
        //表格选项的列
        const tableColumn = ["keyword", "jumpPage", "jumpParam", "weight", "validTime", "deadTime", "status", "createTime", "updateTime"];
        // 选中的行
                ...
                switch (type) {
                    case "allEnable": // 批量启用
                        tableSelectionRows.map((item) => {
                            ids.push(item.id);
                            names.push(item.name);
                            recommendNames.push(item.recommendName);
                        });
                        ids = ids.join(",");
                        names = names.join(",");
                        recommendNames = recommendNames.join(",");
                        break;
                    default:
                        // 单个启用
                        ids = row.id;
                        names = row.name;
                        recommendNames = row.recommendName;
                }
                enableAllSearchJump({
                    ids: ids,
                    names,
