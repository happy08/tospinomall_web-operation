<template>
    <div class="top-search-form">
        <BmBreadcrumb />
        <BmAlert :title="$t('indexRecommendWords.indexRecommendWordsTips')" icon="warning" class="mb-20" />
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10 between">
            <div>
                <!-- 新增 -->
                <el-button v-permission="'operationalSettings_indexRecommendWords_add'" type="primary" @click="handleAddEdit('add')">+{{ $t("status.add") }}</el-button>
                <!-- 功能操作 -->
                <span class="mlr-30">{{ $t("status.functionalOperation") }}:</span>
                <!-- 批量删除 -->
                <el-button v-permission="'operationalSettings_indexRecommendWords_del'" @click="handleAddEdit('allDelete')">{{ $t("filterWords.batchDeletion") }}</el-button>
                <!-- 批量禁用 -->
                <el-button v-permission="'operationalSettings_indexRecommendWords_disable'" @click="handleAddEdit('allDisable')">{{ $t("filterWords.batchDisable") }}</el-button>
                <!-- 批量启用 -->
                <el-button v-permission="'operationalSettings_indexRecommendWords_enable'" @click="handleAddEdit('allEnable')">{{ $t("filterWords.batchEnable") }}</el-button>
            </div>
            <div class="flex">
                <!-- 导出 -->
                <el-button v-permission="'operationalSettings_indexRecommendWords_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
                <BmTableOption table="filterWordsTable" :column="tableColumn" class="ml-10" />
            </div>
        </div>
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <!-- 首页推荐词 -->
            <BmTableColumn :label="$t('indexRecommendWords.name')" prop="name">
                <template #default="scope">{{ scope.row.name }}</template>
            </BmTableColumn>
            <!-- 状态 -->
            <BmTableColumn :label="$t('filterWordsTable.status')" prop="status">
                <template #default="scope">{{ scope.row.statusName }}</template>
            </BmTableColumn>
            <!-- 添加时间 -->
            <BmTableColumn :label="$t('filterWordsTable.createTime')" prop="createTime">
                <template #default="scope">
                    {{ scope.row.createTime }}
                </template>
            </BmTableColumn>
            <!-- 更新时间 -->
            <BmTableColumn :label="$t('filterWordsTable.updateTime')" prop="updateTime">
                <template #default="scope">
            ...
            <BmTableColumn :label="$t('table.operation')" fixed="right" align="center">
                <template #default="scope">
                    <!--启用-->
                    <el-button v-permission="'operationalSettings_indexRecommendWords_enable'" v-if="scope.row.status == 1" @click.stop="handleAddEdit('enable', scope.row)" type="text">{{
                        $t("status.enable")
                    }}</el-button>
                    <!--禁用-->
                    <el-button v-permission="'operationalSettings_indexRecommendWords_disable'" v-if="scope.row.status == 0" @click.stop="handleAddEdit('disable', scope.row)" type="text">{{
                        $t("status.forbidden")
                    }}</el-button>
                    <!-- 编辑 -->
                    <el-button v-permission="'operationalSettings_indexRecommendWords_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">{{ $t("status.edit") }}</el-button>
                    <!-- 删除 -->
                    <el-button v-permission="'operationalSettings_indexRecommendWords_del'" @click.stop="handleAddEdit('delete', scope.row)" type="text">{{ $t("status.delete") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
...
import { indexRecommendWords, enableAllIndexRecommendWords, deleteAllIndexRecommendWords } from "@/api/base";
...
export default {
    ...
    setup() {
        ...
        let tableData = ref([]);
        //表格选项的列
        const tableColumn = ["name", "status", "createTime", "updateTime"];
        // 选中的行
        ...
        const getListPage = () => {
            searchForm.pageNum = pageNum.value;
            indexRecommendWords(searchForm)
                .then((res) => {
        ...
        const handleAddEdit = (type, row) => {
            ...
            if (type == "allDisable" || type == "disable") {
                ...
                deleteAllIndexRecommendWords(ids)
                    .then((res) => {
                        initData();
                    })
                    .catch(() => {});
