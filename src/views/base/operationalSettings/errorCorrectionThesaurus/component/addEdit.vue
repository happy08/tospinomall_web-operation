<template>
    <BmDialog v-model:visible="visible" :title="title" :confirm="confirm" :cancel="cancel" :width="600">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="80px">
            <!-- 过滤词 -->
            <el-form-item :label="$t('filterWordsTable.name')" prop="name">
                <el-input v-model="form.name" :placeholder="$t('pleaseEnter')"></el-input>
            </el-form-item>
            <!-- 替换内容 -->
            <el-form-item :label="$t('errorWordsTable.replaceName')" prop="replaceName">
                <el-input v-model="form.replaceName" :placeholder="$t('pleaseEnter')"></el-input>
            </el-form-item>
            <!-- 状态 -->
...
import { addErrorCorrectionThesaurus, editErrorCorrectionThesaurus } from "@/api/base";
export default {
    name: "AttributeAdd",
    components: {},
    props: {
            ...
            formRef.value.validate((valid) => {
                if (valid) {
                    if (form.value.type == "add") {
                        addErrorCorrectionThesaurus(form.value)
                            .then((res) => {
                                emit("reloadData");
                        ...
                        editErrorCorrectionThesaurus(form.value)
                            .then((res) => {
                                emit("reloadData");
                                emit("update:visible", false, form.value.type);
                                formRef.value.resetFields();
