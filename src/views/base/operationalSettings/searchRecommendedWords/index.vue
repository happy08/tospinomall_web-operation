<template>
    <div class="top-search-form">
        <BmBreadcrumb />
        <BmAlert :title="$t('searchRecommendedWords.searchRecommendedWordsTips')" icon="warning" class="mb-20" />
    ...
    <div class="p-25 bg-white round-4">
        <div class="flex mb-10 between">
            <div>
                <!-- 新增 -->
                <el-button v-permission="'operationalSettings_searchRecommendedWords_add'" type="primary" @click="handleAddEdit('add')">+{{ $t("status.add") }}</el-button>
                <!-- 功能操作 -->
                <span class="mlr-30">{{ $t("status.functionalOperation") }}:</span>
                <!-- 批量删除 -->
                <el-button v-permission="'operationalSettings_searchRecommendedWords_del'" @click="handleAddEdit('allDelete')">{{ $t("filterWords.batchDeletion") }}</el-button>
                <!-- 批量禁用 -->
                <el-button v-permission="'operationalSettings_searchRecommendedWords_disable'" @click="handleAddEdit('allDisable')">{{ $t("filterWords.batchDisable") }}</el-button>
                <!-- 批量启用 -->
                <el-button v-permission="'operationalSettings_searchRecommendedWords_enable'" @click="handleAddEdit('allEnable')">{{ $t("filterWords.batchEnable") }}</el-button>
            </div>
            <div class="flex">
                <!-- 导出 -->
                <el-button v-permission="'operationalSettings_searchRecommendedWords_export'" @click="handleExportExcel">
                    {{ $t("table.exportExcel") }}
                </el-button>
                <!-- 表格选项 -->
                <BmTableOption table="searchRecommendedWordsTable" :column="tableColumn" class="ml-10" />
            </div>
        </div>
        ...
        <BmTable ref="multipleTable" :data="tableData" @selection-change="handleSelectionChange" @row-click="handleRowClick" @row-dblclick="handleRowdblClick" :column="tableColumn">
            ...
            <!-- 搜索词 -->
            <BmTableColumn :label="$t('searchRecommendedWordsTable.name')" prop="name">
                <template #default="scope">{{ scope.row.name }}</template>
            </BmTableColumn>
            <!-- 搜索推荐词 -->
            <BmTableColumn :label="$t('searchRecommendedWordsTable.recommendName')" prop="recommendName">
                <template #default="scope">{{ scope.row.recommendName }}</template>
            </BmTableColumn>
            <!-- 状态 -->
            <BmTableColumn :label="$t('searchRecommendedWordsTable.status')" prop="status">
                <template #default="scope">{{ scope.row.statusName }}</template>
            </BmTableColumn>
            <!-- 添加时间 -->
            <BmTableColumn :label="$t('searchRecommendedWordsTable.createTime')" prop="createTime">
                <template #default="scope">
                    {{ scope.row.createTime }}
                </template>
            </BmTableColumn>
            <!-- 更新时间 -->
            <BmTableColumn :label="$t('searchRecommendedWordsTable.updateTime')" prop="updateTime">
                <template #default="scope">
            ...
            <BmTableColumn :label="$t('table.operation')" fixed="right" align="center">
                <template #default="scope">
                    <!--启用-->
                    <el-button v-permission="'operationalSettings_searchRecommendedWords_enable'" v-if="scope.row.status == 1" @click.stop="handleAddEdit('enable', scope.row)" type="text">{{
                        $t("status.enable")
                    }}</el-button>
                    <!--禁用-->
                    <el-button v-permission="'operationalSettings_searchRecommendedWords_disable'" v-if="scope.row.status == 0" @click.stop="handleAddEdit('disable', scope.row)" type="text">{{
                        $t("status.forbidden")
                    }}</el-button>
                    <!-- 编辑 -->
                    <el-button v-permission="'operationalSettings_searchRecommendedWords_edit'" @click.stop="handleAddEdit('edit', scope.row)" type="text">{{ $t("status.edit") }}</el-button>
                    <!-- 删除 -->
                    <el-button v-permission="'operationalSettings_searchRecommendedWords_del'" @click.stop="handleAddEdit('delete', scope.row)" type="text">{{ $t("status.delete") }}</el-button>
                </template>
            </BmTableColumn>
        </BmTable>
...
import { searchRecommendedWords, enableAllSearchRecommendedWords, deleteAllSearchRecommendedWords } from "@/api/base";
...
export default {
    ...
    setup() {
        ...
        let tableData = ref([]);
        //表格选项的列
        const tableColumn = ["name", "recommendName", "status", "createTime", "updateTime"];
        // 选中的行
        ...
        const getListPage = () => {
            searchForm.pageNum = pageNum.value;
            searchRecommendedWords(searchForm)
                .then((res) => {
        ...
        const handleAddEdit = (type, row) => {
            ...
            if (type == "allDisable" || type == "disable") {
                ...
                enableAllSearchRecommendedWords({
                    ids: ids,
                    names,
                    status: 1
                })
                ...
                deleteAllSearchRecommendedWords({
                    ids,
                    names
                })
                    .then((res) => {
