            ...
            <div>
                ...
                <div class="fl ml-20 flex vcenter">
                    ...
                    <div class="ml-20">
                        ...
                        <!-- 批量审核 -->
                        <el-button v-permission="'goods_checkRecord_audit'" @click="handleBatchAudit">
                            {{ $t("goodsList.batch") }}
                        </el-button>
                    </div>
                </div>
        ...
        >
            ...
            <BmTableColumn :label="$t('table.operation')" width="120" header-align="center" align="center" fixed="right">
                <template #default="scope">
                    <template v-if="scope.row.skus">
                        <div>
                            <el-button v-permission="'goods_checkRecord_look'" @click.stop="handleDetail(scope.row)" type="text">
                                {{ $t("table.detail") }}
                            </el-button>
                        </div>

                        <!-- 审核状态 （1待审核 2已通过 3未通过） -->
                        <el-button v-permission="'goods_checkRecord_audit'" v-if="scope.row.examineStatus === 1" @click.stop="handleBatchAudit(scope.row)" type="text">
                            {{ $t("table.checkGoods") }}
                        </el-button>
                        ...
                        <!-- 查看驳回原因 -->
                        <el-button v-permission="'goods_checkRecord_lookReject'" v-if="scope.row.examineStatus === 3" @click.stop="handleCheck(scope.row)" type="text">
...
export default {
    ...
    setup() {
        ...
        const initData = (pageFlag) => {
            ...
            goodsMallExamineList(params, "new")
                .then((res) => {
        ...
        const searchCount = () => {
            goodsMallExamineStatistical({ type: "type" }, "newcheck")
                .then((res) => {
        ...
        const handleExportExcel = (val) => {
            specModelMallExport(searchForm)
                .then((res) => {
                    exportExcel("GoodsCheckRecord", res);
