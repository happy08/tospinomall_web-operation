    ...
    <div class="p-25 bg-white round-4">
        <div class="flex between mb-10">
            <div>
                ...
                <div class="fl ml-20 flex vcenter">
                    ...
                    <div class="ml-20">
                        ...
                        <!-- 批量审核 -->
                        <el-button v-permission="'goods_check_audit'" @click="handleBatchAudit">
                            {{ $t("goodsList.batch") }}
                        </el-button>
                    </div>
                </div>
        ...
        >
            ...
            <BmTableColumn prop="status" :label="$t('goodsListTable.status')" width="140px">
                <template #default="scope">
                    <!-- 
						spu状态：
						examineStatus： 审核状态 （1待审核 2已通过  3未通过 4撤销审核）
            ...
            <BmTableColumn prop="brandName" :label="$t('goodsListTable.brandName')" width="145">
                <template #default="scope">
                    {{ scope.row.brandName }}
                </template>
            </BmTableColumn>
            ...
            <BmTableColumn :label="$t('table.operation')" width="120" header-align="center" align="center" fixed="right">
                <template #default="scope">
                    <template v-if="scope.row.skus">
                        <div>
                            <el-button v-permission="'goods_check_look'" @click.stop="handleDetail(scope.row)" type="text">
                                {{ $t("table.detail") }}
                            </el-button>
                        </div>

                        <!-- 审核状态 （1待审核 2已通过 3未通过） -->
                        <el-button v-permission="'goods_check_audit'" v-if="scope.row.examineStatus === 1" @click.stop="handleBatchAudit(scope.row)" type="text">
                            {{ $t("table.checkGoods") }}
                        </el-button>
                        ...
                        <el-button v-permission="'goods_check_lookReject'" v-if="scope.row.examineStatus === 3" @click.stop="handleCheck(scope.row)" type="text">
...
import { categoryListTree, brandMallList, specModelMallExport, goodsMallGetStatistical, findByParentId, goodsMallCheckList } from "@/api/goods";
import { getSellerList } from "@/api/store";

import Audit from "./component/audit";
import Check from "./component/check";
import DetailChange from "./component/detailChange";
import { exportExcel } from "@/utils/";
import { dateShortcuts } from "@/utils/dateShortcuts";
import { useRoute } from "vue-router";
export default {
    name: "checkList",
    components: { Audit, Check, DetailChange },
    setup() {
        ...
        const timeTypeList = computed(() => [
            ...
            {
                name: proxy.$t("goodsListTable.examineTime"), //最新审核时间
                value: 2,
                field: "examineTime"
            }
        ]);
        ...
        const searchCount = () => {
            goodsMallGetStatistical({ type: "type" }, "newcheck")
                .then((res) => {
