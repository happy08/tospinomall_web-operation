<template>
    ...
    <div class="p-25 bg-white round-4">
            ...
            <div>
                ...
                <div class="fl flex vcenter">
                    ...
                    <div class="fl ml-20 mb-10">
                        ...
                        <!-- 批量审核 -->
                        <el-button v-permission="'goods_checkChange_audit'" @click="handleBatchAudit">{{ $t("goodsList.batch") }}</el-button>
                    </div>
                </div>
            </div>
        ...
        >
            ...
            <BmTableColumn prop="brandName" :label="$t('goodsListTable.brandName')" width="130"> </BmTableColumn>
            ...
            <BmTableColumn :label="$t('table.operation')" width="140" header-align="center" align="center" fixed="right">
                <template #default="scope">
                    <template v-if="scope.row.skus">
                        <div>
                            <el-button v-permission="'goods_checkChange_look'" @click.stop="handleDetail(scope.row)" type="text">
                                {{ $t("table.detail") }}
                            </el-button>
                        </div>

                        <!-- 审核状态 （1待审核 2已通过 3未通过） -->
                        <el-button v-permission="'goods_checkChange_audit'" v-if="scope.row.examineStatus === 1" @click.stop="handleBatchAudit(scope.row)" type="text">
                            {{ $t("table.checkGoods") }}
                        </el-button>
                        ...
                        <el-button v-permission="'goods_checkChange_lookReject'" v-if="scope.row.examineStatus === 3" @click.stop="handleCheck(scope.row)" type="text">
...
export default {
    ...
    setup() {
        ...
        const initData = () => {
            ...
            goodsMallCheckList(params, "update")
                .then((res) => {
        ...
        const searchCount = () => {
            goodsMallGetStatistical({ type: "type" }, "updatecheck")
                .then((res) => {
        ...
        const handleExportExcel = (val) => {
            specModelMallExport(searchForm)
                .then((res) => {
                    exportExcel("GoodsCheckChange", res);
