<template>
    <!-- 店铺列表 -->
    <BmDialog v-model:visible="visible" :title="$t('shop.list')" :confirm="confirm" :cancel="cancel" :width="1000" top="10vh">
        <el-form ref="searchFormRef" :model="searchForm" :inline="true" label-width="60px">
            <el-form-item prop="keywords">
                <el-input v-model="searchForm.keyword" :placeholder="$t('permissionUser.keywordInfo')"> </el-input>
            </el-form-item>
            <el-form-item>
        ...
        <el-tabs class="custom-tabs-green" v-model="activeName" type="card">
            <el-tab-pane name="first">
                ...
                <el-checkbox-group v-model="tableSelectionIds">
                    <BmTable :data="tableData" @row-click="handleRowClick" height="380" class="mb-20">
                        ...
                        <BmTableColumn prop="companyName" :label="$t('shop.companyName')">
                            <template #default="scope">
                                {{ scope.row.umsSellerCompany.name }}
                            </template>
                        </BmTableColumn>
                        <BmTableColumn prop="storeLogoUrl" label="Logo" width="70px">
                            <template #default="scope">
                                <BmImage :src="scope.row.storeLogoUrl" style="width:40px;height:40px" />
                            </template>
                        </BmTableColumn>
                        <BmTableColumn prop="storeName" :label="$t('shop.name')"> </BmTableColumn>
                        <BmTableColumn prop="storeCode" :label="$t('shop.code')"> </BmTableColumn>
                        <BmTableColumn prop="phone" :label="$t('memberListTable.phone')">
                            <template #default="scope">
                                +{{ scope.row.sellerUser.phonePrefix }}
                                {{ scope.row.sellerUser.phone }}
                            </template>
                        </BmTableColumn>
                        <BmTableColumn prop="email" :label="$t('memberListTable.email')">
                            <template #default="scope">
                                {{ scope.row.sellerUser.email }}
                            </template>
                        </BmTableColumn>
                    </BmTable>
                </el-checkbox-group>
            ...
            <el-tab-pane name="second" class="p-0">
                ...
                <BmTable :data="tableSelectionRows" row-key="id" height="400" class="mb-20">
                    ...
                    <BmTableColumn prop="storeLogoUrl" label="Logo">
                        <template #default="scope">
                            <BmImage :src="scope.row.storeLogoUrl" style="width:40px;height:40px" />
                        </template>
                    </BmTableColumn>
